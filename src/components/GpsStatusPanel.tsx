  ...
  return (
    ...
    >
      ...
      <p aria-live="polite" className="text-lg font-bold">
        {status === 'acquiring' && msg.gps.acquiring}
        {status === 'tracking' && !isStale && msg.gps.tracking}
        {status === 'tracking' && isStale && msg.gps.staleLabel}
      </p>

      {position && (
        <dl className="mt-2 grid grid-cols-2 gap-x-4 gap-y-1 text-base">
          <dt className="text-slate-600 dark:text-slate-300">{msg.gps.accuracyLabel}</dt>
          <dd
          ...
          >
            ...
            {degraded ? msg.gps.accuracyPoor : msg.gps.accuracyGood})
          </dd>

          <dt className="text-slate-600 dark:text-slate-300">{msg.gps.lastUpdate}</dt>
